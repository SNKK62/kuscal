    ...
    Ok(match (value, target) {
        ...
        _ => {
            return Err(TypeCheckError::new(
                format!("{:?} cannot be coerced to {:?}", value, target),
                span,
            ))
        }
    })
        ...
        If(cond, true_branch, false_branch) => {
            ...
            if let Some(false_branch) = false_branch {
                ...
                binary_op_type(&true_type, &false_type).map_err(|_| {
                    ...
                    TypeCheckError::new(
                        format!(
                            "Conditional expression doen't have the \
...
fn type_check<'src>(
    ...
    for stmt in stmts {
        match stmt {
            ...
            Statement::Break => {
                // TODO: now, break with values like Rust is not supported
            }
            Statement::Continue => (),
        }
    }
